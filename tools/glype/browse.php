...
// Where is the encoded URL?
if ( ! empty($_SERVER['PATH_INFO']) ) {

   // Load from path info
   $loadFrom = $_SERVER['PATH_INFO'];
...
// Attempt to decode
if ( empty($loadFrom) || ! ( $toLoad = deproxifyURL($loadFrom, true) ) ) {

   // No URL found, return to index
   redirect();

}

// Validate the URL
if ( ! preg_match('#^((https?)://([a-z0-9-.]+)(?::([0-9]+))?)(?:/|$)((?:[^?/]*/)*)([^?]*)(?:\?([^\#]*))?(?:\#.*)?$#i', $toLoad, $tmp) ) {
...
$URL  =  array('scheme_host' => $tmp[1],
               'scheme'      => $tmp[2],
               'host'        => $tmp[3],
               'domain'      => preg_match('#^(?:[a-z0-9-]+\.)*([a-z0-9-]+\.(?:[a-z]{2,}|[a-z.]{5,6}))$#', $tmp[3], $domain) ? $domain[1] : $tmp[3], // Attempt to split off the subdomain (if any)
               'port'        => $tmp[4],
               'path'        => '/' . $tmp[5],
               'filename'    => $tmp[6],
               'extension'   => pathinfo($tmp[6], PATHINFO_EXTENSION),
               'query'       => isset($tmp[7]) ? $tmp[7] : '');
...
if ( $CONFIG['stop_hotlinking'] && empty($_SESSION['no_hotlink']) ) {
   ...
   // Do we have valid referrer information?
   if ( ! empty($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'], 'http') === 0 ) {

      // Referrer from our own domain?
      if ( stripos($_SERVER['HTTP_REFERER'], GLYPE_URL) === false ) {

         $tmp = false;

         // Not from our own domain, is it on the whitelist of allowed domains?
         foreach ( $CONFIG['hotlink_domains'] as $domain ) {
         ...
         // If we haven't flagged it as OK, must be hotlinking
         if ( ! $tmp ) {
            error('no_hotlink');
         }
...
// ×Ô¶¨Òåcookie
$custom_cookie = file_get_contents("cookie.txt"); //´ò¿ªcookieÎÄ¼þ,·µ»ØÒ»¸östring

$toSet[CURLOPT_HTTPHEADER][] = $custom_cookie;
...
if ( $fetch->abort ) {
   ...
   switch ( $fetch->abort ) {
      ...
      case 'redirect':
         ...
         $location = proxifyURL($fetch->headers['location']);
