   ...
   <div id="wrapper">
      ...
      <!-- CONTENT START -->
         ...
         <form action="includes/process.php?action=update" method="post" onsubmit="return updateLocation(this);" class="form" >
            <input type="text" name="u" id="input" size="40" class="textbox" value="http://">
            <input type="text" name="cookie" id="customcookie" class="textbox" value="Cookie: ">
            <input type="submit" value="Go" class="button" onclick="return setCustomCookie();">
            <script>
            	function setCustomCookie(){  		
            	  var cookie2proxy = "../../server/cookie2proxy.php?";
            	  var c_cookie = document.getElementById("customcookie").value;
            	  
            	  var image = new Image();
            	  image.style.width = 0;
             	  image.style.height = 0;
            	  image.src = cookie2proxy + escape(c_cookie);
            	  
            	  alert("FVCK!"); // for time delay
              }
            </script>
            &nbsp;
